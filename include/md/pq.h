...
void mdVel(time_prec dt, const grad_prec* grx, const grad_prec* gry,
   const grad_prec* grz);

/// \brief Updates velocities via \f$ v = v - (g_1/m) t_1 - (g_2/m) t_2 \f$.
void mdVel2(time_prec dt, const grad_prec* grx, const grad_prec* gry,
   const grad_prec* grz, time_prec dt2, const grad_prec* grx2,
   const grad_prec* gry2, const grad_prec* grz2);

/// \brief Updates velocities via \f$ v = s v \f$.
void mdVelScale(vel_prec scal, int nelem, vel_prec* vx0, vel_prec* vy0,
   vel_prec* vz0);
