...
namespace tinker {
/// \ingroup egv
/// Time scale configuration that assigns a group number to every energy term.
/// Up to 32 different group numbers are supported, from 0 to 31.
///
/// This class must:
///    - implement a C++ style `.at(arg)` member function where \c arg is the name
///      of an energy term;
///    - return the group to which \c arg belongs;
///    - throw an exception if \c arg is an invalid energy name.
///
...
///
/// #### Example 2.
/// There are usually two groups, "high frequency" (fast) and "low frequency" (slow),
/// of energy terms in the RESPA integrator.
/// If one decides to assign 3 and 5 to these two groups, respectively,
/// `tsconfig.at("ebond")` shall return 3 and `tsconfig.at("evdw")` shall return 5.
///
...
/// \ingroup egv
/// \return The default #TimeScaleConfig object. All of the energy terms are in
/// group 0, and can only be accessed via flag 1.
