   ...
   real* restrict dmpik, real* restrict dmpi, real* restrict dmpk, real r, real alphai, real alphak)
{
   ...
   if (diff < eps) {
      ...
      dmpik[4] =
         (1.0f + dampi + 0.5f * dampi2 + dampi3 * div6 + 4.0f * dampi4 * div105 + dampi5 * div210) *
         expi;
      if CONSTEXPR (ORDER > 9) {
         ...
         dmpik[5] = (1.0f + dampi + 0.5f * dampi2 + dampi3 * div6 + 5.0f * dampi4 * div126 +
                       2.0f * dampi5 * div315 + dampi6 * div1890) *
            expi;
      }
   } else {
      real dampi2 = dampi * dampi;
      ...
      dmpik[2] =
         termi * (1 + dampi + dampi2 * div3) * expi + termk * (1 + dampk + dampk2 * div3) * expk;
      dmpik[3] = termi * (1 + dampi + 0.4f * dampi2 + dampi3 * div15) * expi +
         termk * (1 + dampk + 0.4f * dampk2 + dampk3 * div15) * expk;
      dmpik[4] = termi *
            (1 + dampi + 3.0f * dampi2 * div7 + 2.0f * dampi3 * div21 + dampi4 * div105) * expi +
         termk * (1 + dampk + 3.0f * dampk2 * div7 + 2.0f * dampk3 * div21 + dampk4 * div105) *
            expk;
      ...
      if CONSTEXPR (ORDER > 9) {
         ...
         dmpik[5] = termi *
               (1 + dampi + 4.0f * dampi2 * div9 + dampi3 * div9 + dampi4 * div63 +
                  dampi5 * div945) *
               expi +
            termk *
               (1 + dampk + 4.0f * dampk2 * div9 + dampk3 * div9 + dampk4 * div63 +
                  dampk5 * div945) *
               expk;
      }
   }
...
// dfield aplus
SEQ_ROUTINE
inline void damp_aplus3(real r, real pdi, real ddi, real pdk, real ddk, real& restrict scale3,
...
SEQ_ROUTINE
inline void damp_aplus3g(real r, real rr2, real xr, real yr, real zr, real pdi, real ddi, real pdk,
   real ddk, real& restrict scale31, real& restrict scale51, real& restrict scale71,
