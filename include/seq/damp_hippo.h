   ...
   real* restrict dmpj, real r, real ai, real aj)
{
   ...
   if CONSTEXPR (DirOrder >= 9) {
      a5 = a2 * a3, b5 = b2 * b3;
      dmpi[4] = 1
         - (1 + a + 0.5f * a2 + a3 * div6 + (4 * a4 + 0.5f * a5) * div105)
            * expi;
      dmpj[4] = 1
         - (1 + b + 0.5f * b2 + b3 * div6 + (4 * b4 + 0.5f * b5) * div105)
            * expk;
   }
   ...
   real* restrict dmpk, real r, real alphai, real alphak)
{
   ...
   dmpi[3] = (1 + dampi + 0.5f * dampi2 + dampi3 * div6 + dampi4 * div30)
      * expi;
   ...
   if (diff < eps) {
      ...
      dmpik[0] = (1 + (11 * dampi + 3 * dampi2) * div16 + dampi3 * div48)
         * expi;
      dmpik[1] = (1 + dampi + 0.5f * dampi2 + (7 * dampi3 + dampi4) * div48)
         * expi;
      ...
      if CONSTEXPR (ORDER > 9) {
         ...
         dmpik[5] = (1 + dampi + 0.5f * dampi2 + dampi3 * div6 + dampi4 * div24
                       + (dampi5 + dampi6 * div6 + dampi7 * div42
                            + dampi8 * div378)
                          * div120)
            * expi;
      }
      ...
      dmpk[3] = (1 + dampk + 0.5f * dampk2 + dampk3 * div6 + dampk4 * div30)
         * expk;
      ...
      if CONSTEXPR (ORDER > 9) {
         ...
         dmpik[5] = (1 + dampi + 0.5f * dampi2 + dampi3 * div6
                       + 5 * dampi4 * div126 + 2 * dampi5 * div315
                       + dampi6 * div1890)
               * termi2 * expi
...
inline void damp_rep_deprecated(real* restrict dmpik, real r, real rinv,
   real r2, real rr3, real rr5, real rr7, real rr9, real rr11, real dmpi,
   real dmpk)
{
#if TINKER_REAL_SIZE == 8
   real eps = 0.001f;
   ...
   if (diff < eps) dmpi = 0.5f * (dmpi + dmpk);
         ...
         d5s = (dmpk25 * r6 * div945 + 2 * div189 * dmpk24 * r5
                  ...
                  - coef1
                     * (dmpk26 * r5 * div945 + dmpk25 * r4 * div63
                        + dmpk24 * r3 * div9 + 4 * div9 * dmpk23 * r2
                        + dmpk22 * r + dmpk2))
               * dmpi2 * expk
            + (dmpi25 * r6 * div945 + 2 * div189 * dmpi24 * r5
                 ...
                 + coef1
                    * (dmpi26 * r5 * div945 + dmpi25 * r4 * div63
                       + dmpi24 * r3 * div9 + 4 * div9 * dmpi23 * r2
                       + dmpi22 * r + dmpi2))
               * dmpk2 * expi;
      }
   }
...
inline void damp_rep(real* restrict dmpik, real r, real rr1, real r2, real rr3,
   real rr5, real rr7, real rr9, real rr11, real ai /* dmpi */,
   real aj /* dmpk */)
{
   real pfac = 2 / (ai + aj);
   pfac = pfac * pfac;
   ...
   d3s -= d2
      * (f3d * 3 * c2 * ((c - 3) * c - d2)
         + d2 * (f4d * (3 * (2 - c) * c2 + (c + 1) * d2) + f5d * c2d2));
   d3s *= rr7 * inv15;
   d3s *= expmc;
   ...
   d4s += d2
      * (f3d * 2 * c3 * (6 * (c + 1) - 2 * c2 + 3 * d2)
         + d2
            * (f4d * 2 * c2 * (3 * (c - 3) * c - 2 * d2)
               + f5d * d2 * (4 * (2 - c) * c2 + (c + 1) * d2) + f6d * c2 * d4));
   d4s *= rr9 * inv105;
   d4s *= expmc;
