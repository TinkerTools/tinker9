...
, const Spatial::SortedAtoms* restrict sorted
...
EXTRA_KERNEL_PARAMS)
{
    ...
    for (int ii = ithread; ii < nexclude; ii += blockDim.x * gridDim.x) {
        ...
        KERNEL_LOAD_EXCLUDE_XYZ_I
        KERNEL_LOAD_EXCLUDE_PARAMS_I
    ...
    for (int iw = iwarp; iw < nakpl; iw += nwarp) {
        ...
        for (int j = 0; j < WARP_SIZE; ++j) {
            ...
            KERNEL_SCALED_FULL_INTERACTION
    ...
    KERNEL_SUM_ENERGY
    KERNEL_SUM_VIRIAL
} // generated by ComplexKernelBuilder 1.0
'''
...
def load_scale_parameter(ptype, stem, input):
    if ptype in ['real_const_array']:
        v = 'real {} = {}[ii];'.format(stem, input)
        return v
    elif ptype in ['real2_const_array']:
        v = ''
        v = v + 'real {}0 = {}[ii][0];'.format(stem, input)
        v = v + 'real {}1 = {}[ii][1];'.format(stem, input)
        return v
    else:
...
def ik_force(iork, op, lst):
    ...
    for it in lst:
        ...
        if op == 'decl':
            ...
            if shared:
                suffix = '[threadIdx.x]'
            v = v + '{}{}{} = {};'.format(prefix, name, suffix, src)
        elif op == 'init_exclude_xyz':
            if shared:
                suffix = '[threadIdx.x]'
            v = v + '{}{}{} = {}[{}{}];'.format(prefix, name, suffix, it['from'], prefix, iork)
        elif op == 'init_xyz':
            if shared:
                suffix = '[threadIdx.x]'
            v = v + '{}{}{} = sorted[{}{}].{};'.format(prefix, name, suffix, prefix, iork, it['from'])
        elif op == 'load':
            t = it['type']
            if shared:
                suffix = '[srclane + threadIdx.x - ilane]'
            v = v + '{} {} = {}{}{};'.format(t, name, prefix, name, suffix)
        elif op == 'load_exclude':
            t = it['type']
            if shared:
                suffix = '[threadIdx.x]'
            v = v + '{} {} = {}{}{};'.format(t, name, prefix, name, suffix)
...
def generate(yaml_file):
    ...
    k4, v4 = 'KERNEL_LOAD_EXCLUDE_XYZ_I', ''
    ...
    if kcfg in config.keys():
        ...
        v4 = v4 + ik_force('i', 'load_exclude', vcfg)
    ...
    k5, v5 = 'KERNEL_LOAD_EXCLUDE_PARAMS_I', ''
    ...
    if kcfg in config.keys():
        ...
        v5 = v5 + ik_force('i', 'load_exclude', vcfg)
