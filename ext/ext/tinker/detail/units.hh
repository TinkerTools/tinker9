#pragma once

#include "macro.h"

TINKER_NAMESPACE_BEGIN namespace units {
const double avogadro = 6.02214076e+23;
const double lightspd = 2.99792458e-2;
const double boltzmann = 0.831446262e0;
const double gasconst = 1.987204259e-3;
const double elemchg = 1.602176634e-19;
const double vacperm = 8.854187817e-12;
const double emass = 5.4857990907e-4;
const double planck = 6.62607015e-34;
const double joule = 4.1840e0;
const double ekcal = 4.1840e+2;
const double bohr = 0.52917721067e0;
const double hartree = 627.5094736e0;
const double evolt = 27.21138602e0;
const double efreq = 2.194746314e+5;
const double coulomb = 332.063713e0;
const double debye = 4.80321e0;
const double prescon = 6.85684112e+4;
} TINKER_NAMESPACE_END
