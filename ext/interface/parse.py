...
CMAKE_LIBTINKER = '''cmake_minimum_required (VERSION 2.8)

project (TinkerInterface LANGUAGES Fortran {C_OR_CPP})

add_library (tinkerObjF OBJECT
{FORT_FILES})

add_library (tinkerObj{LIB_SUFFIX} OBJECT {C_FILES})

if (${CMAKE_Fortran_COMPILER_ID} STREQUAL GNU)
set_target_properties (tinkerObj{LIB_SUFFIX} PROPERTIES INCLUDE_DIRECTORIES "${PROJECT_SOURCE_DIR}/../include/tinker/gfortran")
elseif (${CMAKE_Fortran_COMPILER_ID} STREQUAL Intel)
set_target_properties (tinkerObj{LIB_SUFFIX} PROPERTIES INCLUDE_DIRECTORIES "${PROJECT_SOURCE_DIR}/../include/tinker/ifort")
endif ()

add_library (tinkerFTo{LIB_SUFFIX} STATIC $<TARGET_OBJECTS:tinkerObjF> $<TARGET_OBJECTS:tinkerObj{LIB_SUFFIX}>)'''
...
def printShell(lang: str, files: List[str]) -> None:
    def cmakeContents(lang: str, files: FileInfoLists) -> str:
        if lang == 'c':
            c_or_cpp = 'C'
            lib_suffix = 'C'
        elif lang == 'cpp':
        ...
        for f in files.subroutine_files:
            ...
            lines.append(relDir + stem + ext)
        if lang == 'cpp':
            c_files: str = 'tinker/routines.cpp tinker/modcpp.cpp'
        elif lang == 'c':
            c_files: str = 'tinker/routines.c'
        contents: str = CMAKE_LIBTINKER.format(C_OR_CPP=c_or_cpp, LIB_SUFFIX=lib_suffix, FORT_FILES='\n'.join(lines), C_FILES=c_files,
            CMAKE_Fortran_COMPILER_ID='{CMAKE_Fortran_COMPILER_ID}', PROJECT_SOURCE_DIR='{PROJECT_SOURCE_DIR}')
    ...
    if len(files):
        ...
        if lang == 'cpp':
            print('echo \'{}\' > CMakeLists.txt'.format(cmakeContents('cpp', file_lists)))
        elif lang == 'c':
            print('echo \'{}\' > CMakeLists.txt'.format(cmakeContents('c', file_lists)))
        print(ShellPart2)
