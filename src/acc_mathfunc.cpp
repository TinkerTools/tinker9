...
template <class T>
T reduce_sum_acc_tmpl(const T* gpu_a, int cpu_n) {
  T val = 0;
...
int reduce_sum(const int* a, int n) { return reduce_sum_acc_tmpl(a, n); }

float reduce_sum(const float* a, int n) { return reduce_sum_acc_tmpl(a, n); }

double reduce_sum(const double* a, int n) { return reduce_sum_acc_tmpl(a, n); }

unsigned long long reduce_sum(const unsigned long long* a, int n) {
  return reduce_sum_acc_tmpl(a, n);
}

template <class T>
void reduce_sum2_acc_tmpl(T* h_ans, int hn, const T* v, int nelem, int neach) {
  assert(hn <= neach);
...
void reduce_sum2(float* h_ans, int hn, const float* v, int nelem, int neach) {
  reduce_sum2_acc_tmpl(h_ans, hn, v, nelem, neach);
}
...
template <class T>
T dotprod_acc_tmpl(const T* gpu_a, const T* gpu_b, int cpu_n) {
  T val = 0;
...
template <class T>
void scale_array_acc_tmpl(T* gpu_dst, T scal, int nelem) {
...
void scale_array(float* gpu_dst, float scal, int nelem) {
  scale_array_acc_tmpl(gpu_dst, scal, nelem);
}
