...
int reduce_sum (const int* a, size_t n)
{
   return reduce_sum_tmpl (a, n);
}

float reduce_sum (const float* a, size_t n)
{
   return reduce_sum_tmpl (a, n);
}

double reduce_sum (const double* a, size_t n)
{
   return reduce_sum_tmpl (a, n);
}
...
template <class T>
void reduce_sum2_tmpl (T* __restrict__ h_ans, size_t hn,
                       const T* __restrict__ v, size_t nelem, size_t neach)
{
   assert (hn <= neach);
