...
__global__
void pcg_udir_aplus(int n, const real* restrict polarity,
...
__global__
void pcg_rsd4(int n, const real* restrict polarity_inv, //
              real (*restrict rsd)[3],                  //
...
__global__
void pcg_rsd3(int n, const real* restrict polarity, real (*restrict rsd)[3])
{
   for (int i = ITHREAD; i < n; i += STRIDE) {
      if (polarity[i] == 0) {
         rsd[i][0] = 0;
...
__global__
void pcg_p7(int n, const real* restrict polarity_inv, real (*restrict vec)[3],
...
__global__
void pcg_p8(int n, const real* restrict polarity, //
...
__global__
void pcg_p9(int n, const real* restrict ksum, const real* restrict ksum1,
...
__global__
void pcg_peek2(int n, float pcgpeek, const real* restrict polarity,
...
void induce_mutual_pcg3_cu(real (*uind)[3])
{
   ...
   launch_k1s(g::s0, n, pcg_udir_aplus, n, polarity, udir, field);
   ...
   if (predict) {
      ufield_aplus(uind, field);
      launch_k1s(g::s0, n, pcg_rsd4, n, polarity_inv, rsd, udir, uind, field);
   } else if (dirguess) {
      ufield_aplus(udir, rsd);
   } else {
      darray::copy(g::q0, n, rsd, field);
   }
   launch_k1s(g::s0, n, pcg_rsd3, n, polarity, rsd);


   // initial M r(0) and p(0)
   if (sparse_prec) {
      sparse_precond_build3();
      sparse_precond_apply3(rsd, zrsd);
   } else {
      diag_precond3(rsd, zrsd);
   }
   ...
   while (!done) {
      ...
      ufield_aplus(conj, field);
      launch_k1s(g::s0, n, pcg_p7, n, polarity_inv, vec, conj, field);
      ...
      // u <- u + a p
      // r <- r - a T p
      launch_k1s(g::s0, n, pcg_p8, n, polarity, a, sum, uind, conj, rsd, vec);
      ...
      // calculate/update p
      launch_k1s(g::s0, n, pcg_p9, n, sum, sum1, conj, zrsd);
      ...
      if (done)
         launch_k1s(g::s0, n, pcg_peek2, n, pcgpeek, polarity, uind, rsd);
   }
