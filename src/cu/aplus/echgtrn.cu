...
template <class Ver>
__global__
void echgtrnAplus_cu1(int n, TINKER_IMAGE_PARAMS, CountBuffer restrict nc, EnergyBuffer restrict ec,
...
static void echgtrnAplus_cu2()
{
   ...
   echgtrnAplus_cu1<Ver><<<ngrid, BLOCK_DIM, 0, g::s0>>>(st.n, TINKER_IMAGE_ARGS, nct, ect, vir_ect, dectx, decty,
      dectz, cut, off, st.si1.bit0, nmdwexclude, mdwexclude, mdwexclude_scale, st.x, st.y, st.z, st.sorted, st.nakpl,
      st.iakpl, st.niak, st.iak, st.lst, chgct, dmpct, f);
}

void echgtrnAplus_cu(int vers)
{
   if (vers == calc::v0)
      echgtrnAplus_cu2<calc::V0>();
   else if (vers == calc::v1)
      echgtrnAplus_cu2<calc::V1>();
   else if (vers == calc::v3)
      echgtrnAplus_cu2<calc::V3>();
   else if (vers == calc::v4)
      echgtrnAplus_cu2<calc::V4>();
   else if (vers == calc::v5)
      echgtrnAplus_cu2<calc::V5>();
   else if (vers == calc::v6)
      echgtrnAplus_cu2<calc::V6>();
}
}
