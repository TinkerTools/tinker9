   ...
   const int* restrict lst, real* restrict chgct, real* restrict dmpct, real f)
{
   ...
   for (int ii = ithread; ii < nexclude; ii += blockDim.x * gridDim.x) {
      ...
      if (r2 <= off * off and incl) {
         real r = REAL_SQRT(r2);
         e_prec e, de;
         pair_chgtrn_aplus<do_g>(r, cut, off, scalea, f, alphai, chgi, alphak, chgk,
                           e, de);
         if CONSTEXPR (do_a)
            if (e != 0 and scalea != 0)
               nctl += 1;
...
void echgtrn_aplus_cu2()
{
   ...
   echgtrn_aplus_cu1<Ver><<<ngrid, BLOCK_DIM, 0, g::s0>>>(
...
void echgtrn_aplus_cu(int vers)
{
   if (vers == calc::v0)
      echgtrn_aplus_cu2<calc::V0>();
   else if (vers == calc::v1)
      echgtrn_aplus_cu2<calc::V1>();
   else if (vers == calc::v3)
      echgtrn_aplus_cu2<calc::V3>();
   else if (vers == calc::v4)
      echgtrn_aplus_cu2<calc::V4>();
   else if (vers == calc::v5)
      echgtrn_aplus_cu2<calc::V5>();
   else if (vers == calc::v6)
      echgtrn_aplus_cu2<calc::V6>();
}
}
