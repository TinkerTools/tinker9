...
#include "md.h"
#include "pme.h"
#include "seq_pair_field_aplus.h"
...
template <class ETYP>
__global__
void dfield_aplus_cu1(
   int n, TINKER_IMAGE_PARAMS, real off, const unsigned* restrict dpinfo,
   ...
   const real* restrict dirdamp, real aewald)
{
   ...
   for (int ii = ithread; ii < nexclude; ii += blockDim.x * gridDim.x) {
      ...
      if (r2 <= off * off and incl) {
         pair_dfield_aplus_v2<ETYP>(
            r2, xr, yr, zr, scaleb, ci[klane], dix[klane], diy[klane],
            diz[klane], pdi[klane], ddi[klane], qixx[klane],
            qixy[klane], qixz[klane], qiyy[klane], qiyz[klane], qizz[klane], ck,
            dkx, dky, dkz, pdk, ddk, qkxx, qkxy, qkxz, qkyy, qkyz,
            qkzz, aewald, fidx, fidy, fidz, fkdx, fkdy, fkdz);
      } // end if (include)
   ...
   for (int iw = iwarp; iw < nakpl; iw += nwarp) {
      ...
      for (int j = 0; j < WARP_SIZE; ++j) {
         ...
         if (r2 <= off * off and incl) {
            pair_dfield_aplus_v2<ETYP>(
               r2, xr, yr, zr, scaleb, ci[klane], dix[klane], diy[klane],
               diz[klane], pdi[klane], ddi[klane], 
               qixx[klane], qixy[klane], qixz[klane], qiyy[klane], qiyz[klane],
               qizz[klane], ck, dkx, dky, dkz, pdk, ddk, qkxx, qkxy,
               qkxz, qkyy, qkyz, qkzz, aewald, fidx, fidy, fidz, fkdx, fkdy,
               fkdz);
         } // end if (include)
...
template <class ETYP>
void dfield_aplus_cu(real (*field)[3])
{
   ...
   dfield_aplus_cu1<ETYP><<<ngrid, BLOCK_DIM, 0, g::s0>>>(
      st.n, TINKER_IMAGE_ARGS, off, st.si1.bit0, ndpexclude, dpexclude,
...
void ufield_aplus_cu1(
   ...
   real (*restrict field)[3], const real* restrict pdamp,
   const real* restrict thole, real aewald)
{
   const int ithread = threadIdx.x + blockIdx.x * blockDim.x;
   const int iwarp = ithread / WARP_SIZE;
   const int nwarp = blockDim.x * gridDim.x / WARP_SIZE;
   ...
   for (int ii = ithread; ii < nexclude; ii += blockDim.x * gridDim.x) {
      ...
      if (r2 <= off * off and incl) {
         pair_ufield_aplus_v2<ETYP>(
            r2, xr, yr, zr, scalec, uidx[klane], uidy[klane], uidz[klane],
            pdi[klane], pti[klane], ukdx, ukdy, ukdz, pdk, ptk, 
   ...
   for (int iw = iwarp; iw < nakpl; iw += nwarp) {
      ...
      for (int j = 0; j < WARP_SIZE; ++j) {
         ...
         if (r2 <= off * off and incl) {
            pair_ufield_aplus_v2<ETYP>(
               r2, xr, yr, zr, scalec, uidx[klane], uidy[klane], uidz[klane],
               pdi[klane], pti[klane], ukdx, ukdy, ukdz,
               pdk, ptk, aewald, fidx, fidy, fidz, fkdx, fkdy, fkdz);
         } // end if (include)
...
template <class ETYP>
void ufield_aplus_cu(const real (*uind)[3], real (*field)[3])
{
...
{
   darray::zero(g::q0, n, field);
   ufield_aplus_cu<NON_EWALD>(uind, field);
}
}
