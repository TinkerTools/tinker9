...
// ck.py Version 2.0.3
__global__
void sparse_precond_cu6(int n, TINKER_IMAGE_PARAMS, real off,
...
void sparse_precond_apply3_cu(const real (*rsd)[3], real (*zrsd)[3])
{
   const auto& st = *uspatial_v2_unit;
   const real off = switch_off(switch_usolve) + st.buffer;


   launch_k1s(g::s0, n, sparse_precond_cu5, //
              rsd, zrsd, polarity, n, udiag);

   int ngrid = get_grid_size(BLOCK_DIM);
   sparse_precond_cu6<<<ngrid, BLOCK_DIM, 0, g::s0>>>(
