...
std::string cxxCompilerName()
{
   ...
   if (__PGIC__ <= 19)
      n = format("pgc++ %d.%d.%d", __PGIC__, __PGIC_MINOR__, __PGIC_PATCHLEVEL__);
   else
      n = format("nvc++ %d.%d.%d", __PGIC__, __PGIC_MINOR__, __PGIC_PATCHLEVEL__);

#elif defined(TINKER_APPLE_CLANG)
   n = format("clang++ %d.%d.%d (xcode)", __clang_major__, __clang_minor__, __clang_patchlevel__);

#elif defined(TINKER_LLVM_CLANG)
   n = format("clang++ %d.%d.%d (llvm)", __clang_major__, __clang_minor__, __clang_patchlevel__);
