...
void velocity_verlet(int istep, time_prec dt_ps)
{
   ...
   if (userat) {
      darray::copy(async_queue, n, rattle_xold, xpos);
      darray::copy(async_queue, n, rattle_yold, ypos);
      darray::copy(async_queue, n, rattle_zold, zpos);
   }
   // s += v * dt
   propagate_pos(dt_ps);
   if (userat)
...
void langevin_piston(time_prec dt, virial_prec press)
{
   ...
   darray::copy(async_queue, n, vx, leapfrog_vxold);
   darray::copy(async_queue, n, vy, leapfrog_vyold);
   darray::copy(async_queue, n, vz, leapfrog_vzold);
   ...
   darray::copy(async_queue, n, leapfrog_vxold, vx);
   darray::copy(async_queue, n, leapfrog_vyold, vy);
   darray::copy(async_queue, n, leapfrog_vzold, vz);
   // vx = 0.5 * (vxnew + vxold)
   eksum_new = 0.0;
...
void lpiston_npt(int istep, time_prec dt_ps)
{
   ...
   if (istep == 1 || com_done) {
      ...
      if (istep == 1) {
         darray::copy(async_queue, n, leapfrog_x, xpos);
         darray::copy(async_queue, n, leapfrog_y, ypos);
         darray::copy(async_queue, n, leapfrog_z, zpos);
         ...
         darray::copy(async_queue, n, xpos, leapfrog_x);
         darray::copy(async_queue, n, ypos, leapfrog_y);
         darray::copy(async_queue, n, zpos, leapfrog_z);
         if (userat) {
            shake(dt_ps, xpos, ypos, zpos, leapfrog_x, leapfrog_y, leapfrog_z);
            darray::copy(async_queue, n, leapfrog_x, xpos);
            darray::copy(async_queue, n, leapfrog_y, ypos);
   ...
   if (userat) {
      darray::copy(async_queue, n, vx, leapfrog_vx);
      darray::copy(async_queue, n, vy, leapfrog_vy);
      darray::copy(async_queue, n, vz, leapfrog_vz);
