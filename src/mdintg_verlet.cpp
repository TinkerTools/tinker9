...
// Langevin Piston
vel_prec *leapfrog_vxold, *leapfrog_vyold, *leapfrog_vzold; // old halftime velocity
double press;
...
void langevin_piston(time_prec dt, virial_prec press)
{
   ...
   for (int i = 1; i <=3; ++i) {
      ...
      if(pnhf_pre == 0.0) pnhf_pre = pnhf_lp;
   ...
   // vx = 0.5 * (vxnew + vxold)
   eksum_new = 0.0;
   propagate_velocity_lp3(vx, vy, vz,
                          leapfrog_vx, leapfrog_vy, leapfrog_vz,
                          leapfrog_vxold, leapfrog_vyold, leapfrog_vzold, eksum_new); // eksum_new => vx
...
void lpiston_npt(int istep, time_prec dt_ps)
{
   ...
   if (istep ==1 || com_done) {
      ...
      propagate_velocity(-t2, leapfrog_vx, leapfrog_vy, leapfrog_vz, vx, vy, vz,
                         gx, gy, gz);
      // v(0.5) = v(0) + a(0)*dt/2
      propagate_velocity(t2, leapfrog_vxold, leapfrog_vyold, leapfrog_vzold, vx, vy, vz,
                         gx, gy, gz);
   ...
   propagate_velocity(dt_ps, leapfrog_vx, leapfrog_vy, leapfrog_vz,
                      leapfrog_vxold, leapfrog_vyold, leapfrog_vzold, gx, gy, gz);
   ...
   propagate_velocity_lp3(vx, vy, vz,
                          leapfrog_vx, leapfrog_vy, leapfrog_vz,
                          leapfrog_vxold, leapfrog_vyold, leapfrog_vzold, eksum_vx);
   ...
   press = (stress[0][0] + stress[1][1] + stress[2][2]) / 3;
   press += (eksum + eksum_old) * factor / 3;
   swap_v_lf(leapfrog_vx, leapfrog_vy, leapfrog_vz,
             leapfrog_vxold, leapfrog_vyold, leapfrog_vzold);
}
