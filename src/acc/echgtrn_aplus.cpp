...
#include "seq_pair_chgtrn_aplus.h"
...
template <class Ver>
void echgtrn_aplus_acc1()
{
   assert(ctrntyp == chgtrn_t::COMBINED);
   ...
   for (int i = 0; i < n; ++i) {
      ...
      for (int kk = 0; kk < nmlsti; ++kk) {
         ...
         if (r2 <= off2) {
            ...
            pair_chgtrn_aplus<do_g>(r, cut, off, 1, f, alphai, chgi, alphak, chgk, e,
                              de);
   ...
   for (int ii = 0; ii < nmdwexclude; ++ii) {
      ...
      if (r2 <= off2) {
         real r = REAL_SQRT(r2);
         MAYBE_UNUSED real e, de;
         pair_chgtrn_aplus<do_g>(r, cut, off, mscale, f, alphai, chgi, alphak, chgk,
                           e, de);

         if CONSTEXPR (do_a) {
            real e1, de1;
            pair_chgtrn_aplus<do_g>(r, cut, off, 1, f, alphai, chgi, alphak, chgk, e1,
                              de1);
...
void echgtrn_aplus_acc(int vers)
{
   if (vers == calc::v0)
      echgtrn_aplus_acc1<calc::V0>();
   else if (vers == calc::v1)
      echgtrn_aplus_acc1<calc::V1>();
   else if (vers == calc::v3)
      echgtrn_aplus_acc1<calc::V3>();
   else if (vers == calc::v4)
      echgtrn_aplus_acc1<calc::V4>();
   else if (vers == calc::v5)
      echgtrn_aplus_acc1<calc::V5>();
   else if (vers == calc::v6)
      echgtrn_aplus_acc1<calc::V6>();
}
}
