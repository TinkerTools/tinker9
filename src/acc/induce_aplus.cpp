...
void sparse_precond_apply3_acc(const real (*rsd)[3], real (*zrsd)[3])
{
   #pragma acc parallel loop independent async\
               deviceptr(polarity,rsd,zrsd)
   for (int i = 0; i < n; ++i) {
 ...
 *
 * M = preconditioner
 * T = inv_alpha + Tu, -Tu = ufield_aplus
 *
 * r(0) = E - T u(0)
 * p(0) (or conj(0)) = M r(0)
 * ------------------------------
...
void induce_mutual_pcg3(real (*uind)[3])
{
   ...
   if (pltfm_config & CU_PLTFM)
      induce_mutual_pcg3_cu(uind);
   else
#endif
      induce_mutual_pcg3_acc(uind);
}


void ulspred_save3_acc(const real (*restrict uind)[3])
{
...
void ulspred_sum3_acc(real (*restrict uind)[3])
{
   if (nualt < maxualt)
      return;
