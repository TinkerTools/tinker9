...
template <class ETYP>
void dfield_aplus_acc1(real (*field)[3])
{
   real aewald = 0;
   real off;
   ...
   for (int ii = 0; ii < ndpexclude; ++ii) {
      ...
      if (r2 <= off2) {
         real3 fid = make_real3(0, 0, 0);
         real3 fkd = make_real3(0, 0, 0);
         pair_dfield_aplus<NON_EWALD>(
            r2, xr, yr, zr, dscale, ci, dix, diy, diz, pdi, ddi, 
...
template <class ETYP>
void ufield_aplus_acc1(const real (*uind)[3], real (*field)[3])
{
   real aewald = 0;
   real off;
   if CONSTEXPR (eq<ETYP, EWALD>()) {
   ...
   for (int i = 0; i < n; ++i) {
      ...
      for (int kk = 0; kk < nmlsti; ++kk) {
         ...
         if (r2 <= off2) {
            real3 fid = make_real3(0, 0, 0);
            real3 fkd = make_real3(0, 0, 0);
            pair_ufield_aplus<ETYP>(
               r2, xr, yr, zr, 1, uindi0, uindi1, uindi2, pdi, pti, 
               uind[k][0], uind[k][1], uind[k][2], pdamp[k], thole[k], 
   ...
   for (int ii = 0; ii < nuexclude; ++ii) {
      ...
      if (r2 <= off2) {
         real3 fid = make_real3(0, 0, 0);
         real3 fkd = make_real3(0, 0, 0);
         pair_ufield_aplus<NON_EWALD>(
            r2, xr, yr, zr, uscale, uindi0, uindi1, uindi2, pdi, pti, 
            uind[k][0], uind[k][1], uind[k][2], pdamp[k], thole[k], 0,
