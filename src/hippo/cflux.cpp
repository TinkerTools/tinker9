...
TINKER_FVOID2(cu, 1, acc, 1, alterchg);
void alterchg()
{
   TINKER_FCALL2(cu, 1, acc, 1, alterchg);
}
...
TINKER_FVOID2(cu, 1, acc, 1, dcflux, int, grad_prec*, grad_prec*, grad_prec*, VirialBuffer);
void dcflux(int vers, grad_prec* gx, grad_prec* gy, grad_prec* gz, VirialBuffer v)
{
   TINKER_FCALL2(cu, 1, acc, 1, dcflux, vers, gx, gy, gz, v);
}
}
