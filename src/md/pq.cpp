...
namespace tinker {
TINKER_F2VOID(cu, 0, acc, 1, mdPos, time_prec, pos_prec*, pos_prec*, pos_prec*, const vel_prec*,
...
{
   TINKER_F2CALL(cu, 0, acc, 1, mdPos, dt, qx, qy, qz, vlx, vly, vlz);
}

void mdPos(time_prec dt)
{
   TINKER_F2CALL(cu, 0, acc, 1, mdPos, dt, xpos, ypos, zpos, vx, vy, vz);
}

TINKER_F2VOID(cu, 0, acc, 1, mdPosAxbv, pos_prec, pos_prec);
void mdPosAxbv(pos_prec a, pos_prec b)
{
   TINKER_F2CALL(cu, 0, acc, 1, mdPosAxbv, a, b);
}

TINKER_F2VOID(cu, 0, acc, 1, mdPosAxbvAn, pos_prec (*)[3], pos_prec (*)[3]);
void mdPosAxbvAn(pos_prec (*a)[3], pos_prec (*b)[3])
{
   TINKER_F2CALL(cu, 0, acc, 1, mdPosAxbvAn, a, b);
}

TINKER_F2VOID(cu, 0, acc, 1, mdDebugPosNorm, pos_prec, time_prec, const vel_prec*, const vel_prec*,
   const vel_prec*);
void mdDebugPosNorm(pos_prec poseps, time_prec dt, //
   const vel_prec* vx, const vel_prec* vy, const vel_prec* vz)
{
   TINKER_F2CALL(cu, 0, acc, 1, mdDebugPosNorm, poseps, dt, vx, vy, vz);
}
}

namespace tinker {
TINKER_F2VOID(cu, 0, acc, 1, mdVel, time_prec, vel_prec*, vel_prec*, vel_prec*, const grad_prec*,
   const grad_prec*, const grad_prec*);
void mdVel(time_prec dt, const grad_prec* grx, const grad_prec* gry, const grad_prec* grz)
{
   TINKER_F2CALL(cu, 0, acc, 1, mdVel, dt, vx, vy, vz, grx, gry, grz);
}

TINKER_F2VOID(cu, 0, acc, 1, mdVelB, time_prec, vel_prec*, vel_prec*, vel_prec*, //
...
{
   TINKER_F2CALL(cu, 0, acc, 1, mdVelB, dt, vlx, vly, vlz, vlx0, vly0, vlz0, grx, gry, grz);
}

TINKER_F2VOID(cu, 0, acc, 1, mdVel2, time_prec, const grad_prec*, const grad_prec*,
...
{
   TINKER_F2CALL(cu, 0, acc, 1, mdVel2, dt, grx, gry, grz, dt2, grx2, gry2, grz2);
}

TINKER_F2VOID(cu, 0, acc, 1, mdVelAvbf, int, vel_prec, vel_prec, vel_prec*, vel_prec*, vel_prec*,
...
{
   TINKER_F2CALL(cu, 0, acc, 1, mdVelAvbf, nrespa, a, b, vx, vy, vz, gx1, gy1, gz1, gx2, gy2, gz2);
}

TINKER_F2VOID(cu, 0, acc, 1, mdVelAvbfAn, int, vel_prec (*)[3], vel_prec (*)[3], vel_prec*,
