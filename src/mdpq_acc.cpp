...
{
   const vel_prec s = scale;
   const energy_prec ekcal_inv = 1.0 / units::ekcal;
   energy_prec ekn =0 , eko =0;

   #pragma acc parallel loop independent async\
               deviceptr(mass,vx_lp,vy_lp,vz_lp,vxnew_lp,vynew_lp,vznew_lp,vxold_lp,vyold_lp,vzold_lp)
   for (int i = 0; i < n; ++i) {
     vel_prec vxf = 0.5f * (vx_lp[i] + vxold_lp[i]);
     vel_prec vyf = 0.5f * (vy_lp[i] + vyold_lp[i]);
     vel_prec vzf = 0.5f * (vz_lp[i] + vzold_lp[i]);
     vx_lp[i] = vxnew_lp[i] + s * vxf;
     vy_lp[i] = vynew_lp[i] + s * vyf;
     vz_lp[i] = vznew_lp[i] + s * vzf;
                      ...
                      pos_prec* xold_lp, pos_prec* yold_lp, pos_prec* zold_lp, double scale)
{
   ...
   #pragma acc parallel loop independent async\
               deviceptr(x_lp,y_lp,z_lp,vx_lp,vy_lp,vz_lp,xold_lp,yold_lp,zold_lp)
   for (int i = 0; i < n; ++i) {
                                ...
                                pos_prec* xold_lp, pos_prec* yold_lp, pos_prec* zold_lp)
{
   ...
   #pragma acc parallel loop independent async\
               deviceptr(vx_lp,vy_lp,vz_lp,x_lp,y_lp,z_lp,xold_lp,yold_lp,zold_lp)
   for (int i = 0; i < n; ++i) {
