...
// __global__
// void empole_chgpen_cu1(EMPOLEPARAS, const Spatial::SortedAtom* restrict
// sorted,
...
//       }
//       if CONSTEXPR (do_v)
//          atomic_add(vtlxx, vtlxy, vtlxz, vtlyy, vtlyz, vtlzz, vir_em,
//          offset);
//    } // end for (iw)
// }
...
// template <class Ver>
// __global__
// void empole_chgpen_cu2(EMPOLEPARAS, const real* restrict x, const real*
// restrict y,
   ...
   real* restrict pval, const real* restrict palpha, real aewald, real f)
{
   ...
   for (int ii = ithread; ii < nexclude; ii += blockDim.x * gridDim.x) {
      ...
      if (r2 <= off * off and incl) {
         ...
         if CONSTEXPR (do_v) {
            vemtlxx += -xr * pgrad.frcx;
            vemtlyx += -0.5f * (yr * pgrad.frcx + xr * pgrad.frcy);
            vemtlzx += -0.5f * (zr * pgrad.frcx + xr * pgrad.frcz);
            vemtlyy += -yr * pgrad.frcy;
            vemtlzy += -0.5f * (zr * pgrad.frcy + yr * pgrad.frcz);
            vemtlzz += -zr * pgrad.frcz;
         }
      } // end if (include)
...
void empole_chgpen_cu()
{
   ...
   //               bufsize, nem, em, vir_em, demx, demy, demz, trqx, trqy,
   //               trqz, TINKER_IMAGE_ARGS, pcore, pval, palpha, off2, f,
   //               rpole, // st.sorted, st.niak, st.iak, st.lst, n, aewald);
   ...
   //               bufsize, nem, em, vir_em, demx, demy, demz, trqx, trqy,
   //               trqz, TINKER_IMAGE_ARGS, pcore, pval, palpha, off2, f,
   //               rpole, // x, y, z, nmexclude, mexclude, mexclude_scale);
   // }
